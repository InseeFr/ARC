...
(function($) {
    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        var defaults = { // start off with some default settings
                ...
                label:             'Filtre:',           // text to precede the filter input tag
                minRows:           6,                   // don't show the filter on tables with less than this number of rows
                placeholder:       'recherche sur la table', // HTML5 placeholder text for the filter field
        ...
        var doFiltering = function(table, q) { // handle the actual table filtering
                ...
                //$("ajout").show().addClass(settings.visibleClass);
            }; // doFiltering()
