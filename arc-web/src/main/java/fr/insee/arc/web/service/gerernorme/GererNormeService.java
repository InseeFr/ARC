package fr.insee.arc.web.service.gerernorme;
...
@Component
public class GererNormeService extends ArcWebGenericService<NormManagementModel> implements IDbConstant {
	...
	@Autowired
	private GererNormeServiceTemp gererNormeServiceTemp;
	...
	public void putAllVObjects(NormManagementModel model) {		
		...
		putVObject(getViewNorme(),
				t -> gererNormeServiceTemp.initializeViewNorme(t, dataObjectService.getView(ViewEnum.IHM_NORME)));
		//
		putVObject(getViewCalendrier(), t -> gererNormeServiceTemp.initializeViewCalendar(t, getViewNorme(),
				dataObjectService.getView(ViewEnum.IHM_CALENDRIER) ));
		//
		putVObject(getViewJeuxDeRegles(), t -> gererNormeServiceTemp.initializeViewRulesSet(t, getViewCalendrier(),
				dataObjectService.getView(ViewEnum.IHM_JEUDEREGLE) ));
		//
		putVObject(getViewModules(), t -> gererNormeServiceTemp.initializeViewModules(t, getViewJeuxDeRegles()));
		//
		putVObject(getViewChargement(), t -> gererNormeServiceTemp.initializeChargement(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_CHARGEMENT_REGLE) ));
		//
		putVObject(getViewNormage(), t -> gererNormeServiceTemp.initializeNormage(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_NORMAGE_REGLE) ));
		//
		putVObject(getViewControle(), t -> gererNormeServiceTemp.initializeControle(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_CONTROLE_REGLE) ));
		//
		putVObject(getViewFiltrage(), t -> gererNormeServiceTemp.initializeFiltrage(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_FILTRAGE_REGLE) ));
		//
		putVObject(getViewMapping(), t -> gererNormeServiceTemp.initializeMapping(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_MAPPING_REGLE)  ));
		//
		putVObject(getViewExpression(), t -> gererNormeServiceTemp.initializeExpression(t, getViewJeuxDeRegles(), getViewModules(),
				dataObjectService.getView(ViewEnum.IHM_EXPRESSION) ));
		//
		putVObject(getViewJeuxDeReglesCopie(), t -> gererNormeServiceTemp.initializeJeuxDeReglesCopie(t, getViewJeuxDeRegles(), getViewModules(),
	...
	public String updateRuleSet(Model model) {
		...
		if (!selection.isEmpty()) {
			for (int i = 0; i < selection.get("etat").size(); i++) {
				...
				if (ConstanteBD.ARC_PROD.getValue().equals(etat)) {
					gererNormeServiceTemp.sendRuleSetToProduction(this.viewJeuxDeRegles,
	...
	public String downloadJeuxDeRegles(Model model, HttpServletResponse response) {
		...
		if (!selection.isEmpty()) {
			...
			requeteRegleChargement.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_CHARGEMENT_REGLE)
					));
			ArcPreparedStatementBuilder requeteRegleNormage = new ArcPreparedStatementBuilder();
			requeteRegleNormage.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_NORMAGE_REGLE)));
			ArcPreparedStatementBuilder requeteRegleControle = new ArcPreparedStatementBuilder();
			requeteRegleControle.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_CONTROLE_REGLE)));
			ArcPreparedStatementBuilder requeteRegleMapping = new ArcPreparedStatementBuilder();
			requeteRegleMapping.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_MAPPING_REGLE)));
			ArcPreparedStatementBuilder requeteRegleFiltrage = new ArcPreparedStatementBuilder();
			requeteRegleFiltrage.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_FILTRAGE_REGLE)));
			ArcPreparedStatementBuilder requeteRegleExpression = new ArcPreparedStatementBuilder();
			requeteRegleExpression.append(gererNormeServiceTemp.recupRegle(this.viewJeuxDeRegles,
					dataObjectService.getView(ViewEnum.IHM_EXPRESSION)));
	...
	public String importChargement(Model model, MultipartFile fileUploadLoad) {		
		gererNormeServiceTemp.uploadFileRule(getViewChargement(), viewJeuxDeRegles, fileUploadLoad);
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String importNormage(Model model, MultipartFile fileUploadStructurize) {
		...
		gererNormeServiceTemp.uploadFileRule(getViewNormage(), viewJeuxDeRegles, fileUploadStructurize);
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String importControle(Model model, MultipartFile fileUploadControle) {
		gererNormeServiceTemp.uploadFileRule(getViewControle(), viewJeuxDeRegles, fileUploadControle);
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String viderChargement(Model model) {
		...
		gererNormeServiceTemp.emptyRuleTable(this.viewJeuxDeRegles,
				dataObjectService.getView(ViewEnum.IHM_CHARGEMENT_REGLE));
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String importExpression(Model model, MultipartFile fileUploadExpression) {
		gererNormeServiceTemp.uploadFileRule(getViewExpression(), viewJeuxDeRegles, fileUploadExpression);
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String importFiltrage(Model model, MultipartFile fileUploadFilter) {
		...
		gererNormeServiceTemp.uploadFileRule(this.viewFiltrage, this.viewJeuxDeRegles, fileUploadFilter);
		return generateDisplay(model, RESULT_SUCCESS);
	}
	...
	public String importMapping(Model model, MultipartFile fileUploadMap) {
		gererNormeServiceTemp.uploadFileRule(getViewMapping(), viewJeuxDeRegles, fileUploadMap);
		return generateDisplay(model, RESULT_SUCCESS);
	}
