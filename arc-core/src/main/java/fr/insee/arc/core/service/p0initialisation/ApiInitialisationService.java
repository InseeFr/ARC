...
public class ApiInitialisationService extends ApiService {
	...
	private void rebuildFileSystem() throws ArcException {
		...
		String nomTableArchive = TableNaming.dbEnv(envExecution) + "pilotage_archive";
		...
		if (UtilitaireDao.get(0).hasResults(null, FormatSQL.tableExists("arc.ihm_entrepot"))) {
			...
			if (entrepotList != null) {
				for (String entrepot : entrepotList) {
					...
					String fullEnvDir = FileSystemManagement.directoryEnvRoot(repertoire, this.envExecution);
	...
	private void nettoyerTablePilotage(Connection connexion, String envExecution) throws ArcException {
		...
		String nomTablePilotage = TableNaming.dbEnv(envExecution) + "pilotage_fichier";
		String nomTableArchive = TableNaming.dbEnv(envExecution) + "pilotage_archive";
		...
		try {
			...
			for (int i = 0; i < r.length; i++) {
				...
				tableImage = FormatSQL
						.temporaryTableName(TableNaming.dbEnv(anExecutionEnvironment) + parameterTable);
	...
	private ArcPreparedStatementBuilder requeteListAllTablesEnvTmp(String env) {
		...
		for (int i = 2; i < phase.length; i++) {
			...
			requete.append(FormatSQL.tableExists(TableNaming.dbEnv(this.envExecution) + phase[i] + "$%$tmp$%"));
			requete.append(" UNION ALL ");
			requete.append(
					FormatSQL.tableExists(TableNaming.dbEnv(this.envExecution) + phase[i] + "\\_%$tmp$%"));
		}
		return requete;
	}
	...
	private void synchroniserEnvironmentByPilotage(Connection connexion, String envExecution) throws ArcException {
		...
		// les DBAs
		DatabaseMaintenance.maintenanceDatabaseClassic(connexion, envExecution);
	...
	private static void deleteUnusedDataRecords(Connection executorConnection, ListIdSourceInPilotage listIdSourceInPilotage, List<String> envTables, TraitementPhase phase, TraitementEtat etat, List<String> providedIdSourceToDelete) throws ArcException
	{
		...
		List<String> envTablesWithRecords = envTables.stream().filter(nomTable -> PhaseOperations.extractPhaseFromTableName(nomTable).equals(phase) && PhaseOperations.extractEtatFromTableName(nomTable).equals(etat)).collect(Collectors.toList());
		...
		for (String dataTable : envTablesWithRecords) {
			...
			{
				idSourceInDataTableThatShouldntBe= PhaseOperations.selectIdSourceOfDataTable(executorConnection, dataTable);
	...
	{		
		...
		List<String> templateDataTablesThatCanBeDropped = PhaseOperations.selectPhaseDataTablesFoundInEnv(executorConnection, envExecution)
				.stream().filter(nomTable -> !PhaseOperations.extractPhaseFromTableName(nomTable).equals(TraitementPhase.MAPPING)).collect(Collectors.toList());
	...
	private static void dropUnusedDataTables(Connection coordinatorConnexion, Connection executorConnection, String tablePilotage, List<String> templateDataTablesThatCanBeDropped, List<String> providedIdSourceToDrop) throws ArcException
	{
		...
		for (String templateDataTable : templateDataTablesThatCanBeDropped) {
			...
			else
			{
				...
				List<String> childDataTables = PhaseOperations.selectAllChildrenPhaseDataTables(executorConnection, templateDataTable);
				...
				for (String childDataTable : childDataTables) {
					...
					String idSource = PhaseOperations.selectIdSourceOfChildDataTable(executorConnection, childDataTable);
					String etape = PilotageOperations.accessSelectEtapeForIdSource(coordinatorConnexion, tablePilotage, phase, etat, idSource);
