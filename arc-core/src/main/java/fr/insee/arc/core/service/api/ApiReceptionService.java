...
public class ApiReceptionService extends ApiService {
	...
	private GenericBean moveAndCheckClientFiles(int fileSizeLimit, int maxNumberOfFiles) {
		...
		try {
			...
			if (!entrepotList.isEmpty()) {
				archivesContent = moveAndCheckUntilLimit(fileSizeLimit, maxNumberOfFiles,
						entrepotList.get("id_entrepot"));
			}
		} catch (Exception ex) {
	 ...
	 */
	private GenericBean moveAndCheckUntilLimit(int fileSizeLimit, int maxNumberOfFiles,
			ArrayList<String> entrepotIdList) throws ArcException {
		...
		for (String d : entrepotIdList) {
			...
			String dirArchive = ApiReceptionService.directoryReceptionEntrepotArchive(this.directoryRoot,
					this.envExecution, d);
...
//								&& !f.getName().endsWith(ApiService.SUFFIXE_TEMP_FILE_ORIADE)
							&& !matcher.matches() && !f.getName().equals(ApiService.FICHIER_MISE_EN_PRODUCTION)) {
						if (fileSize > fileSizeLimit || fileNb > maxNumberOfFiles) {
							setReportNumberOfObject(fileNb);
							break;
						}
						...
						for (int i = 1; i < Integer.MAX_VALUE; i++) {
							...
							if (i > 1) {
								fname = ManipString.substringBeforeFirst(fname, ".") + "#" + i + "."
										+ ManipString.substringAfterFirst(fname, ".");
							}
							...
							if (!fileOutArchive.exists()) {
								...
								if (isArchive) {
									...
									try {
										Files.copy(Paths.get(f.getAbsolutePath()),
												Paths.get(fileOutArchive.getAbsolutePath()));
									} catch (IOException exception) {
										throw new ArcException(exception, ArcExceptionMessage.FILE_COPY_FAILED, f,
												fileOutArchive);
									}
									...
									try {
										Files.copy(Paths.get(fileOutArchive.getAbsolutePath()),
												Paths.get(fOut.getAbsolutePath()));
									} catch (IOException exception) {
										throw new ArcException(exception, ArcExceptionMessage.FILE_COPY_FAILED,
												fileOutArchive, fOut);
									}
								...
								//
								GenericBean archiveContentTemp = checkArchiveFiles(
										new File[] { new File(dirOut + File.separator + d + "_" + fname) });
	...
	/**
	 * If there is any error, all files are marked KO. Otherwise, all files are
	 * marked OK.
	 */
		...
		for (ArrayList<String> fileInfo : archiveContent) {
			if (erreur > 0) {
				...
				if (fileInfo.get(GENERIC_BEAN_HEADERS.indexOf(GB_REPORT)) == null) {
					fileInfo.set(GENERIC_BEAN_HEADERS.indexOf(GB_REPORT),
							TraitementRapport.INITIALISATION_FICHIER_OK_ARCHIVE_KO.toString());
				}
			} else {
			...
			if (!g.content.isEmpty()) {
				...
				for (int i = 0; i < g.content.size(); i++) {
					...
					if (type.equals(TraitementTypeFichier.A.toString())) {
						String dirOut = ApiReceptionService.directoryReceptionEtat(this.directoryRoot,
								this.envExecution, TraitementEtat.valueOf(etat));
					...
					if (type.equals(TraitementTypeFichier.D.toString())) {
						...
						// de pilotage
						String dirOut = ApiReceptionService.directoryReceptionEtatOK(this.directoryRoot,
								this.envExecution);
	...
	private void insertPilotage(StringBuilder requete, String tablePilotage, String originalContainer,
			String newContainer, String v_container, String fileName, String etat, String rapport) {
		Date d = new Date();
		...
		try {
			...
			if (listContainerDoublons != null) {
				for (ArrayList<String> z : content) {
					...
					if (container != null) {
						z.set(headers.indexOf(GB_VCONTAINER),
								listVersionContainerDoublons.get(listContainerDoublons.indexOf(container)));
					}
				}
			}
		} catch (ArcException ex) {
	...
	public static String directoryReceptionEntrepot(String rootDirectory, String env, String entrepot) {
		return ServiceFileSystemManagement.directoryPhaseEntrepot(rootDirectory, env, TraitementPhase.RECEPTION,
				entrepot);
	}

	public static String directoryReceptionEntrepotArchive(String rootDirectory, String env, String entrepot) {
		return ServiceFileSystemManagement.directoryPhaseEntrepotArchive(rootDirectory, env, TraitementPhase.RECEPTION,
				entrepot);
	}
