...
commit;do $$ begin alter table arc.ihm_chargement_regle add CONSTRAINT ihm_chargement_regle_type_fichier_fkey FOREIGN KEY (type_fichier) REFERENCES arc.ext_type_fichier_chargement(id) ON DELETE CASCADE ON UPDATE cascade ; EXCEPTION WHEN OTHERS then end; $$;
commit;do $$ begin CREATE TRIGGER tg_insert_chargement BEFORE INSERT ON arc.ihm_chargement_regle FOR EACH ROW EXECUTE PROCEDURE arc.insert_controle(); exception when others then end; $$;
