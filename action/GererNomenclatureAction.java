...
public class GererNomenclatureAction implements SessionAware, IConstanteCaractere, IDbConstant {
    ...
    private void importNomenclatureDansBase() throws Exception {
        ...
        if (nouvelleNomenclature == null || "".equals(nouvelleNomenclature)) {
            this.viewListNomenclatures.setMessage("Vous devez selectionner une nomenclature pour l'importation.");
            return;
        }

        // Ouverture du fichier
        if (this.fileUpload == null || "".equals(this.fileUpload)) {
	    ...
	    try {
	        ...
	        String[] colonnes = rd.readLine().replaceAll("\"", "").split(";");
	        String[] types = rd.readLine().replaceAll("\"", "").split(";");
...
//                UtilitaireDao.get(poolName).executeBlock(null, insert + ";");
...
//        UtilitaireDao.get(poolName).executeBlock(null, insert + ";");
    ...
    private void createTableIhmSchemaNmclIfNotExists() throws SQLException {
        ...
        createTableIhmSchemaNmcl.append("\n );");
        UtilitaireDao.get(poolName).executeRequest(null, createTableIhmSchemaNmcl);
    }
