...
/**
 * Launch class for the ARC application
 * 07/08/2015
 * Version for performance tests and pre-production
 * @author Manu
 */
public class ARCLauncher {
	...
	// Do wee keep the intermediary data in the database or do we export them as file in the export folder?
	// false in production 
...
//	private static String version="ARC-DADS v008 09/05/2018";
	...
	// For the current batch, the total size of the envelopes processed must not exceed a certain limit
	...
	// Maximum lag behind phases: if a phase is deltaStepAllowed steps ahead, it waits for the next one to finish before proceeding
	...
	/**
	 * 	notificationStart
	 * variable pilotage de l'ordonnancement du batch. Voir modalité pluinitialisations bas
...
//	private static Integer heureInitialisationMax= 5;
	...
	public static class InitializeThread extends Thread {
		...
		@Override
		public void run() {
			InitializeBatch c = new InitializeBatch(mapParam.get(ENV), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , maxReceptionSizeInMB, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
	...
	public static class ReceiveThread extends Thread {
		...
		@Override
		public void run() {
			ReceiveBatch c = new ReceiveBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , maxReceptionSizeInMB, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
			this.report = c.report;
		}
	}
	...
	public static class LoadThread extends Thread {
		...
		@Override
		public void run() {
			LoadBatch c = new LoadBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , numberOfFiles, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
			this.report = c.report;
	...
	public static class NormalizeThread extends Thread {
		...
		@Override
		public void run() {
			NormalizeBatch c = new NormalizeBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , numberOfFiles, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
	...
	public static class ControlThread extends Thread {
		...
		@Override
		public void run() {
			ControlBatch c = new ControlBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , numberOfFiles, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
	...
	public static class FilterThread extends Thread {
		...
		@Override
		public void run() {
			FilterBatch c = new FilterBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , numberOfFiles, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
	...
	public static class MapperThread extends Thread {
		...
		@Override
		public void run() {
			MapperBatch c = new MapperBatch(mapParam.get("env"), mapParam.get(ENV_EXECUTION), mapParam.get(REPERTOIRE) , numberOfFiles, keepInDatabase?null:mapParam.get(NUMLOT));
			c.execute();
				...
				if (productionOn()) {
					...
					do {
						...
						if ((!dejaEnCours && dLastInitialize.compareTo(dNow) < 0) || !production)
						{
							...
							initializer.start();
							if (production) {
								message("Initialisation : "+(int)initializer.report.nbLines+" e : "+initializer.report.duree+" ms");
							}
						...
						if (productionOn) {
							...
							if (production) {
								message("Reception : " + (int)receiver.report.nbLines + " e : " + receiver.report.duree + " ms");
							}
							...
							do {
								...
								if (!loader.isAlive() && step.contains(0) && Collections.frequency(step,1) < deltaStepAllowed) {
									loader = new LoadThread();
									loader.start();
								}

								if (!normalizer.isAlive() && step.contains(1) && Collections.frequency(step,2) < deltaStepAllowed) {
									normalizer = new NormalizeThread();
									normalizer.start();
								}

								if (!controller.isAlive() && step.contains(2) && Collections.frequency(step,3) < deltaStepAllowed) {
									controller = new ControlThread();
									controller.start();
								}

								if (!filter.isAlive() && step.contains(3) && Collections.frequency(step,4) < deltaStepAllowed) {
								...
								if (loader.report.nbLines > 0) {
									message(step + " Chargement : " + (int)loader.report.nbLines + " l : " + loader.report.duree + " ms");
								...
								if (normalizer.report.nbLines > 0) {
									message(step + " Normage : " + (int)normalizer.report.nbLines + " l : " + normalizer.report.duree + " ms");
									if (!step.contains(1)) {
										step.add(1);
									}
								...
								if (controller.report.nbLines > 0) {
									message(step + " Controle : " + (int)controller.report.nbLines + " l : " + controller.report.duree + " ms");
									if (!step.contains(2)) {
										step.add(2);
									}
								...
								if (filter.report.nbLines > 0) {
									message(step + " Filtre : " + (int)filter.report.nbLines + " l : " + filter.report.duree + " ms");
									if (!step.contains(3)) {
										step.add(3);
									}
