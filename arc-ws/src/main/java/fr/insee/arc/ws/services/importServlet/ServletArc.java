...
public class ServletArc extends HttpServlet {
	...
	public boolean securityAccessAndTracing(HttpServletRequest request, HttpServletResponse response,
		...
		String clientRealName = ManipString
				.substringBeforeFirst(ManipString.substringAfterFirst(clientDeclared, "."), "_");
		...
		if (result.isEmpty() || result.get("is_secured").isEmpty()) {
			...
			LoggerHelper.error(LOGGER,
					"The host " + request.getRemoteHost() + "/"+request.getRemoteAddr()+"/"+hostName+ " has not been allowed to retrieved data of (" + familyName
							+ "," + clientRealName + "). Check the family norm interface to declare it.");
			return false;
		}
		...
		{
			// if query is not secured and the host had been declared as secured, return forbidden
			if (!requestSecured) {
		...
		// log the access
		query = new PreparedStatementBuilder();
		query.append("DELETE FROM arc.ihm_webservice_log  where event_timestamp < current_timestamp - INTERVAL '1 YEAR';");
		query.append("INSERT INTO arc.ihm_webservice_log (id_famille, id_application, host_allowed, event_timestamp) ");
		query.append("SELECT " + query.quoteText(familyName) + ", " + query.quoteText(clientRealName) + ", " + query.quoteText(hostName) + ", current_timestamp;");
