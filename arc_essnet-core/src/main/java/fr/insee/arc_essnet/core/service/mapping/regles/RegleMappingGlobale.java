package fr.insee.arc_essnet.core.service.mapping.regles;
...
import fr.insee.arc_essnet.core.service.mapping.TableMapping;
import fr.insee.arc_essnet.core.service.mapping.VariableMapping;
...
public class RegleMappingGlobale extends AbstractRegleMappingSimple {
    ...
    public void deriver() throws Exception {
        ...
        this.expressionSQL = UtilitaireDao.get(DbConstant.POOL_NAME).getString(this.connexion, requete);
    }
    ...
    public void deriverTest() throws Exception {
        ...
        while (matcher.find()) {
            ...
            returned.append(AbstractPhaseService.dbEnv(this.environnement) + intermediaire.substring(start + TWO, matcher.end() - ONE));
            end = matcher.end();
        }
        returned.append(intermediaire.substring(end));
        this.expressionSQL = UtilitaireDao.get(DbConstant.POOL_NAME).getString(this.connexion, returned);
    }
    ...
    private String obtenirRequeteExecutable() {
        ...
        for (TableMapping table : this.ensembleTableMapping) {
            returned = returned.replace(tokenTable(table.getNomTableCourt()), AbstractPhaseService.dbEnv(this.environnement) + table);
        }
        return returned;
    }
