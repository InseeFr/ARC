package fr.insee.arc_essnet.core.service.handler;
...
public class XMLHandlerCharger4 extends org.xml.sax.helpers.DefaultHandler {
	...
	try {
	    ...
	    LoggerHelper.error(LOGGER, ex, "startElement()");
	    // e.printStackTrace();
	...
	// la condition de ***REMOVED*** est assez spéciale
	...
	if (this.closedTag.equals(this.currentTag) && this.leafPossible) {
		...
		// throw new SAXParseException("Fichier XML non pris en charge : feuille en
		// doublon de rubrique : " + closedTag, "", "", 0, 0);

		// insertQueryBuilder(requete,tempTableI, fileName, lineCols, lineIds,
		// lineValues);
	...
	try {
	    ...
	    // }
	    // this.requete.append(" TABLE " + this.tempTableI + "
	    // "+FormatSQL.WITH_NO_VACUUM+" as select * from " + this.tempTableA + " where
	    // 1=0;");

	    this.colDist.put(-1, 0);

	} catch (Exception ex) {
	    LoggerHelper.error(LOGGER, ex, "instartDocumentdex()");
	}
	...
	if (o == null) {
	    ...
	    // {
	    // throw new
	    // SAXParseException("Fichier XML non pris en charge : rubrique placée
	    // diferemment dans l'arbre xml : "+currentTag,"","",0,0);
	    // }
	...
	// && updated==false
		...
		// LoggerDispatcher.info("Insertion :" + requete.length() + " caractères ",
		// logger);
	...
	// //
	// System.out.println("attribut "+i+" - "+atts.getName(i)+" :
	// "+atts.getValue(i));
	// }
    }
     ...
     * @throws SAXParseException
     */
    public void insertQueryBuilder(StringBuilder aRequete, String tempTableI, String fileName, List<Integer> lineCols,
	...
	for (int i = 0; i < lineCols.size(); i++) {
	    ...
	    // System.out.println(doNotinsert.get(tree.get(lineCols.get(i)))==null ||
	...
	LoggerDispatcher.info("Create XML join", LOGGER);

	// construction de la requete de jointure
	StringBuilder req = new StringBuilder();

	int[][] arr = Format.getTreeArrayByDistance(this.tree, this.colDist);
	// StringBuilder reqCreate = new StringBuilder("WITH noview as (select false as
	// n) \n");
	StringBuilder reqCreate = new StringBuilder(" \n");

	StringBuilder reqInsert = new StringBuilder();
	reqInsert.append(
		" INSERT INTO {table_destination} (id,id_source,date_integration,id_norme,validite,periodicite");
	...
	for (int i = 0; i < arr.length; i++) {
		...
		// String leafMaxWhenNull = Format.getLeafsMaxWhenNull2(arr[i][1], arr,
		// this.colData, this.allCols);
		// reqCreate.append("CREATE TEMPORARY TABLE t_" + allCols.get(arr[i][1]) + " as
		// (select distinct i_" + allCols.get(arr[i][1]) + " m_" +
		// allCols.get(arr[i][1]) + " ");
		// if (arr[i][0] >= 0) {
