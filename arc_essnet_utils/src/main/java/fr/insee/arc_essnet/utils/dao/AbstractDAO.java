...
import fr.insee.arc_essnet.utils.sqlengine.CompoundName;
import fr.insee.arc_essnet.utils.sqlengine.ContextName;
import fr.insee.arc_essnet.utils.sqlengine.DefaultNaming;
import fr.insee.arc_essnet.utils.sqlengine.Naming;
import fr.insee.arc_essnet.utils.sqlengine.StringToken;
import fr.insee.arc_essnet.utils.sqlengine.Tokens;
import fr.insee.arc_essnet.utils.sqlengine.model.SQLTable;
import fr.insee.arc_essnet.utils.utils.FormatSQL;
import fr.insee.arc_essnet.utils.utils.LoggerHelper;

public abstract class AbstractDAO<T>
        implements IQueryHandler, IQueryExecutor, IWrapper<Connection>, IDelegate<IQueryHandler>, AutoCloseable
{
    ...
    public final Function<ResultSet, Long> getId = (res) -> {
        try
        {
            return res.getLong(this.getIdEntity());
    ...
    /**
     * Pour traiter le resultat d'une requete qui ne contient qu'une seule colonne de résultat.
     */
	...
	public static final Function<ResultSet, ContextName> GET_CONTEXTNAME = (res) -> {
        try
        {
        	return new CompoundName(
        				new DefaultNaming(Tokens.TOK_SCHEMA
        						, new StringToken(res.getString("table_schema"))
        						,Tokens.TOK_TABLE_NAME
        						,new StringToken(res.getString("table_name"))
        					)
    ...
    {
        StringBuilder requete = FormatSQL.getAllReccordsFromATable(getNomTableEntity().name());
     ...
     * @param aLimit
     *            le nombre maximum d'enregistrements renvoyés
     * @param anOffset
     *            le nombre à partir duquel on renvoie les enregistrements
     * @return
     * @throws Exception
     */
     ...
     * @throws Exception
     */
    public final List<T> getCuttedList(String anOrderByColumn, String aCondition, Integer aLimit, Integer anOffset) throws Exception
    {
	 StringBuilder requete = new StringBuilder();
	if (StringUtils.isEmpty(aCondition)) {
	    
	    requete = FormatSQL.simpleSelectRequest(null,getNomTableEntity().name(),anOrderByColumn,true, aLimit, anOffset);
	} else {
	    requete = FormatSQL.simpleSelectRequest(null,getNomTableEntity().name(),anOrderByColumn,true, aLimit, anOffset,aCondition);
